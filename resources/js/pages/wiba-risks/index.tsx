...
const breadcrumbs: BreadcrumbItem[] = [{ title: 'WIBA Risks', href: '/wiba-risks' }];
...
export default function WibaRiskNotesIndex({ riskNotes, filters }: Props) {
    ...
    const pageTitle = useMemo(() => {
        if (!q && !status) return 'WIBA Risks';
        return 'WIBA Risks • Filtered';
    }, [q, status]);
    ...
    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            ...
            <div className="flex h-full flex-1 flex-col gap-4 overflow-x-auto rounded-xl p-4">
                <Heading title="WIBA Risks" description="Underwriting workflow for Work Injury Benefits Act" />
                <div className="flex justify-end">
                    <Button asChild variant="secondary" size="sm">
                        <Link href="/wiba-risks/create">+ New Risk Note</Link>
                    </Button>
                </div>
                            ...
                            <div className="flex items-end gap-2">
                                ...
                                <Button
                                    ...
                                    onClick={() => {
                                        setQ('');
                                        setStatus('');
                                        router.get('/wiba-risks', {}, { replace: true });
                                    }}
                                >
                                    Reset
                                </Button>
                ...
                {emptyState ? (
                    <Card>
                        <CardContent className="py-8 text-sm text-muted-foreground">No WIBA risk notes found.</CardContent>
                    </Card>
                ) : (
                    <Card>
                                            ...
                                            <TableRow key={r.id}>
                                                ...
                                                <TableCell className="text-right">
                                                    <Button asChild variant="ghost" size="sm">
                                                        <Link href={`/wiba-risks/${r.id}`}>View</Link>
                                                    </Button>
                                                </TableCell>
                                            </TableRow>
                                        );
