...
return new class extends Migration
{
    public function up(): void
    {
        Schema::table('motor_policy_details', function (Blueprint $table) {
            $table->foreignId('binder_version_id')
                ->nullable()
                ->after('policy_id')
                ->constrained('binder_versions')
                ->nullOnDelete();

            $table->string('insurer_policy_number', 80)->nullable()->after('engine_size');
            $table->string('internal_policy_number', 80)->nullable()->after('insurer_policy_number');
            $table->string('customer_id', 80)->nullable()->after('internal_policy_number');
            ...
            $table->date('time_on_risk_start_date')->nullable()->after('pin_number');
    ...
    {
        Schema::table('motor_policy_details', function (Blueprint $table) {
            $table->dropConstrainedForeignId('binder_version_id');
            $table->dropColumn([
